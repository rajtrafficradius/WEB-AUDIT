...
case "${1:-web}" in
  ...
  web)
    ...
    if [ "${EMBED_ANALYSIS_WORKER:-1}" = "1" ]; then
      # Queue order matters: with queue_order_strategy=priority the worker
      # drains analysis (live audits) before render (background rebuilds).
      celery -A app worker --loglevel=${LOG_LEVEL:-INFO} --queues=analysis,render --concurrency=${EMBED_WORKER_CONCURRENCY:-1} --max-tasks-per-child=20 &
    fi
