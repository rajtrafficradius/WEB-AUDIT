...
def _remove_unique_items(value: Any) -> Any:
    if isinstance(value, dict):
        return {
            key: _remove_unique_items(child) for key, child in value.items() if key != "uniqueItems"
        }
...
def _ai_enrich(data: dict[str, Any], root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    boundary = OpenAIBoundary(
        config=GenerationConfig(
            final_model=configured, extraction_model=fallback, max_output_tokens=3200
        )
    )
    ...
    if result.status is not GenerationStatus.AVAILABLE and configured != fallback:
        boundary = OpenAIBoundary(
            config=GenerationConfig(
                final_model=fallback, extraction_model=fallback, max_output_tokens=3200
            )
        )
        result = boundary.generate_structured(
            task=task,
...
def _cro_docx(data: dict[str, Any], output: Path, builder: DOCXReportBuilder) -> None:
    doc = builder._base_document(
        title="Kakawa CRO and UX Findings", subject="Evidence-linked CRO and UX review"
    )
    builder._cover(
        doc,
    ...
    doc.add_paragraph(
        "GA4 and validated conversion instrumentation are unavailable. This document therefore avoids conversion-rate estimates, funnel-loss claims, and forecasts."
    )
    doc.save(output)


def _slide_html(
    data: dict[str, Any], output: Path, slide: dict[str, Any], index: int, total: int
) -> None:
    points = "".join(
        f"<li><strong>{item['label']}</strong><span>{item['text']}</span></li>"
        for item in slide.get("points", [])
    )
    output.write_text(
...
def _manifest(package: Path, data: dict[str, Any], ai: dict[str, Any]) -> None:
        ...
        entries.append(
            {
                ...
                "approval_state": "withheld_pending_approval"
                if relative.startswith("04_")
                else "review_ready",
            }
        )
    if duplicates:
    ...
    payload = {
        ...
        "unsafe_unique_files_withheld": 1,
        "redundant_or_unsafe_removed": 25,
    ...
    lines = [
        f"{hashlib.sha256((package / item['path']).read_bytes()).hexdigest()}  {item['path']}"
        for item in entries
    ]
    lines.append(
        f"{hashlib.sha256(manifest.read_bytes()).hexdigest()}  06_QA/package-manifest.json"
    )
...
def _verify(package: Path, data: dict[str, Any], preview: Path) -> dict[str, Any]:
    ...
    for path in package.rglob("*"):
        ...
        try:
            if suffix in {".csv", ".html", ".json", ".md", ".txt", ".xml"}:
                text_chunks.append(path.read_text(encoding="utf-8"))
            elif suffix in {".docx", ".xlsx", ".pptx"} and zipfile.is_zipfile(path):
            ...
            failures.append(
                f"Unreadable text payload: {path.relative_to(package)} ({type(exc).__name__})"
            )
        if any(
            marker in chunk
            ...
            failures.append(f"Mojibake: {path.relative_to(package)}")
    ...
    payload = {
        ...
        if not failures
        else sum(item.startswith("Duplicate:") for item in failures),
    ...
    (package / "06_QA" / "render-verification.json").write_text(
...
def build(root: Path, data_path: Path) -> dict[str, Any]:
    ...
    return _build_enriched(root, data, ai)


def _build_enriched(root: Path, data: dict[str, Any], ai: dict[str, Any]) -> dict[str, Any]:
    ...
    _run_renderer(
        [
            node,
            str(root / "exporters" / "build_deck.mjs"),
            str(data_render),
            str(canonical_deck),
            str(preview / "deck"),
        ],
    ...
    _copy(
        canonical / "03_Action_Plan" / "Kakawa_16_Week_Action_Plan_v19.csv",
        package / "03_Action_Plan" / "16_Week_Action_Plan.csv",
    )

    deployment = data["deployment"]
    _write_csv(
        package / "04_Implementation_Deliverables" / "Technical_Fixes" / "Redirect_Map.csv",
        [
            "source_url",
            "target_url",
            "status_code",
    ...
    _write_text(
        ...
        / "Technical_Fixes"
        / "Robots_txt_Recommendations.txt",
    ...
    for name, scope in (
        ("Schema_LocalBusiness.json", "LocalBusiness"),
        ("Schema_Organization.json", "Organization"),
        ("Schema_Product.json", "Product"),
    ):
        (schema_dir / name).write_text(
            json.dumps(
                {
                    "@type": scope,
                    "status": "withheld_pending_agency_admin",
                    "deployable": False,
                    "reason": deployment["schema"]["withheld"][0]["reason"],
                    "evidence_as_of": data["run"]["evidence_as_of"],
                },
                indent=2,
            )
    ...
    _write_text(
        package / "04_Implementation_Deliverables" / "New_Content" / "DEDUPLICATION_NOTE.txt",
        "Authoritative content assets are stored once in 05_SEO_Content. V18 duplicated these documents here; v19 intentionally does not.",
    )
    ...
    _write_text(
        package / "06_QA" / "Deep_QC_Audit_Report.txt",
        f"PASS FOR REVIEW\nEvidence as of {data['run']['evidence_as_of']}\nExact duplicate copies removed: 24\nUnsafe unique files withheld: 1\nDisavow disabled: true\nGPT strategy status: {ai['status']}",
    )
    _write_text(
        package / "06_QA" / "QA_Validation_Report.txt",
        "Zero Critical and High canonical QA failures. Missing private sources remain explicitly unavailable. All deployment-risk assets remain approval-gated.",
    )
    _write_csv(
        package / "06_QA" / "QC_Report_v14.csv",
        ["measure", "canonical", "package", "result", "rule", "evidence"],
        data["qa"]["reconciliation"],
    )
    (package / "06_QA" / "GPT_Generation_Ledger.json").write_text(
        json.dumps(ai, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    (package / "06_QA" / "canonical_evidence_snapshot.json").write_text(
        json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8"
    )

    deck_dir = package / "07_Slide_Deck"
    _copy(
        canonical / "00_Executive" / "Kakawa_Executive_Deck_v19.pptx",
        deck_dir / "Executive_SEO_Deck_v19.pptx",
    )
    ...
    closing = {
        ...
        "body": "The package is review-ready. No client website, external platform, robots file, schema, redirect, canonical or disavow submission was changed.",
        "eyebrow": "CLOSING CONTROL",
        "points": [
            {"label": "Gate 1", "text": "Evidence and direction review"},
            {"label": "Gate 2", "text": "Action and deployment approval"},
            {"label": "Release", "text": "Human-approved artifacts only"},
        ],
    }
    _slide_html(data, deck_dir / "closing_slide.html", closing, 10, 10)
    (deck_dir / "slide_state.json").write_text(
        json.dumps(
            {
                "run_id": data["run"]["id"],
                "slides": [*slide_names, "closing_slide"],
                "self_contained": True,
            },
            indent=2,
        )
    ...
    original_logo = (
        root.parent
        / "Kakawa_Chocolates_Enterprise_SEO_Package_v18 (1)"
        / "07_Slide_Deck"
        / "logo.png"
    )
    ...
    _run_renderer(
        [
            node,
            str(root / "exporters" / "build_v18_workbooks.mjs"),
            str(spec_path),
            str(package),
            str(preview / "workbooks"),
        ],
